...
## 18. Angleichungen aus der Umsetzung (Sync 30.08.2026)

Zwei Entscheidungen hat das Repo aus der Umsetzung heraus getroffen. Beide sind richtig und gelten ab jetzt auch für den Prototyp — sie überschreiben, was in §15 und §16 anders stand.

### 18.1 Das Steuerjahr zeigt Cent

§15 rundete auf Euro. In der Umsetzung stand dadurch **562 €** im Steuerjahr neben **561,60 €** in der PKV-Bilanz — dieselbe Zahl, zwei Darstellungen, und der Sprung zwischen beiden Berichten machte den Widerspruch sichtbar.

**Regel: Beträge, die in ein Formular abgetippt werden, erscheinen auf den Cent.** Das betrifft im Steuerjahr die Kopfzahl, jede Positionszeile, jede Abschnittssumme, die Belegquote, die Schätzzeile, das Druckblatt und die Brücke aus der PKV-Bilanz — **und in der PKV-Bilanz selbst den Eigenanteil**: Hero, Legende und die Zeilen je Leistungserbringer. Der Eigenanteil ist die Zahl, die als Krankheitskosten ins Formular wandert; sie darf nicht auf einem Screen zweimal verschieden dastehen (beim ersten Umbau blieb genau das übrig: Hero „716 €" zwei Zeilen über der Brücke „716,20 €"). Gerundete Euro bleiben richtig, wo es um Größenordnungen geht — Kostentrend, Fixkosten, Vermögensbalken.

### 18.2 Die Kurszeitreihe wächst nur vorwärts

§16 ging davon aus, dass eine Historie abrufbar ist. Der Befund aus der Umsetzung: die frei zugängliche Schnittstelle gibt **keine Vergangenheit** heraus. Der Kurs-Endpunkt liefert den zuletzt festgestellten Kurs, der Historien-Endpunkt antwortet leer — er verlangt eine Signatur, die der Anbieter nur seiner eigenen Oberfläche mitgibt. Sie nachzubauen wäre das Umgehen einer Zugangssperre und ist zu Recht unterblieben.

Folgen für das Design:

- Die Reihe **wächst Tag für Tag mit den eigenen Abrufen**. Ein frisch angebundenes Papier hat keinen Verlauf, sondern einen Punkt.
- Vergangenheit kommt **aus dem eigenen Haus**: Ausführungen (§11.1), Bestandsnachweise (§11.2) und erfasste Positionen tragen alle einen Kurs mit Datum und werden nachgetragen.
- Der Kursverlauf muss deshalb mit **wenigen, ungleich verteilten Punkten** umgehen können — nicht mit einer dichten Tagesreihe. Ein Verlauf aus vier Belegpunkten über zwei Jahre ist der Normalfall, nicht die Ausnahme.
- Was daraus folgt und in §16 noch offen ist: Fremdwährung (der Umrechnungskurs wäre selbst eine Zeitreihe), Splits und Ausschüttungen (ein Split bricht den Verlauf optisch ein, ohne Vermögensverlust), Zweitquelle und Kurs von Hand.

### 18.3 Drei Repo-Befunde, die noch kein Design haben

- **Die steuerliche Einordnung einer Kategorie existiert im Modell, ist aber unsichtbar.** Ohne eingeordnete Kategorie bleiben Handwerkerleistungen und Werbungskosten im Steuerjahr leer — das Feld gehört in den Kategorienschirm (§8d).
- **Entfernung und Arbeitstage** sind nur beim Anlegen eines Arbeitsverhältnisses setzbar, nicht in der Bearbeitenmaske (§8).
- **Der Druck läuft über die Druckfunktion des Browsers** — das Blatt ist gestaltet und in der Vorschau geprüft, aber die realen Ränder hängen am Browser.

---

## 19. Vorsorge: fünf Korrekturen (30.08.2026)

Fünf Befunde aus der Benutzung, alle im selben Bereich — und vier davon mit derselben Ursache: der Detailscreen war für **einen** Vertrag hartkodiert.

### 19.1 Verschachteltes Scrollen (Punkt 1)

Im Bearbeitenformular ließ sich nicht scrollen. Gemessen: `documentElement.scrollHeight` 947 gegen `clientHeight` 540 — die **ganze Seite** war höher als das Fenster, und das Formular hatte *zusätzlich* seinen eigenen Scrollbereich. Zwei geschachtelte Scroller: die erste Geste bewegt die Seite, erst die zweite das Formular. Das fühlt sich an wie „geht nicht".

**Regel: es scrollt genau ein Bereich.** Die App-Hülle wird auf die Fensterhöhe begrenzt — Höhe `min(844px, 100vh − 112px)` statt fester 844 px — und die Außenabstände der Vorschauseite sind gekürzt. Danach gilt auf Dokumentebene `scrollHeight === clientHeight`; nur der Inhaltsbereich scrollt. Für die Umsetzung: die App füllt den Viewport, die Seite selbst hat keinen Scroll.

### 19.2 Zahlungszuordnung über die Vertragsnummer (Punkt 2)

Die zugeordneten Buchungen passten nicht — die Zuordnung lief über den Namen, und der Beispielbeitrag war mit 212,00 € gebucht, während der Vertrag 42,00 €/Monat ausweist.

**Zugeordnet wird ausschließlich über die Vertragsnummer im Verwendungszweck**, normalisiert (Groß/Klein, Leerzeichen und Trennzeichen entfernt). Ein Treffer über den Anbieternamen ist bei mehreren Verträgen desselben Hauses falsch — die Beispieldaten zeigen genau das: vier Verträge, vier Beitragsbuchungen, jede mit ihrer Nummer im Verwendungszweck („BEITRAG 08/2026 VERTRAG 01511104-01 …").

Dazu drei Dinge, die im UI stehen:

- **Die Zuordnungsbegründung an jeder Zeile**: „Vertragsnummer 01511104-01 im Verwendungszweck", darunter der Verwendungszweck im Original.
- **Eine Gegenprobe** unter der Liste: gebuchter Betrag gegen den Beitrag im Vertrag, mit Tag „stimmt" / „prüfen" — weicht er ab, ist einer der beiden Werte veraltet, und der Text sagt das.
- **Ein ehrlicher Leerzustand**: ohne Treffer erklärt der Screen, dass nur über die Nummer zugeordnet wird, und bietet „In Buchungen suchen". Ohne erfasste Nummer sagt er das stattdessen.

### 19.3 Police wird angezeigt (Punkt 3)

Die Dokumentliste war hartkodiert und kannte keine Police. Jetzt führt **jeder Vertrag seine Dokumente** — Police, Nachträge, Statusreporte — mit Typ-Tag (`Police` im Akzent, `Bericht`/`Nachtrag` neutral), Datum und Sprung ins Dokument. Der neueste Bericht ist hervorgehoben. „Beleg hinzufügen" führt in den Scan-Flow.

### 19.4 Angelegte Daten sind editierbar (Punkt 4)

Ursache und Lösung in einem Satz: der Detailscreen liest jetzt **den ausgewählten Vertrag** statt fester Werte, und „Vertrag bearbeiten" öffnet das Formular mit **allen** Rohfeldern (nachgemessen: zehn gefüllte Felder). Das gilt für alle vier Beispielverträge — Kapital-LV, Riester, Bausparen — jeder mit eigener Nummer, eigenem Beitrag, eigenem Ablauf.

Die Regel aus §6b gilt hier doppelt: **Objekte führen ihre Rohfelder, die Anzeige wird daraus gerendert.** Ein hartkodierter Detailscreen ist derselbe Fehler, nur an anderer Stelle.

### 19.5 Wie der erreichte Wert entsteht (Punkt 5)

Neuer Block **„So entsteht der Wert"** direkt unter der Kopfzahl:

| Zeile | Herkunft | Betrag |
| --- | --- | --- |
| Rückkaufswert | Statusreport 31.07.2025 | 18.373,87 € |
| + Ansammlungsguthaben | Statusreport 31.07.2025 | 2.107,65 € |
| **= Erreichter Wert** | zählt so ins Vermögen | **20.481,52 €** |

Beide Bestandteile sind eigene Formularfelder; ist einer gefüllt, rechnet die App die Summe und schreibt sie ins Wertfeld. Führt ein Vertrag nur einen Wert (Bausparen), erscheint **eine** Zeile — keine Summe aus einem Summanden.

Zwei Sätze darunter tragen die eigentliche Aussage:

- **Fortschreibung**: „schreibt den Wert bis zum nächsten Bericht unverändert fort — er wird nicht verzinst und nicht geschätzt." Das ist die Grundlage der gestrichelten Kante im Vermögensbericht (§17.1).
- **Ausschluss**: „Nicht enthalten: Bewertungsreserven und Schlussüberschüsse — der Bericht weist sie als nicht garantiert aus, deshalb zählen sie in keiner Vermögenssumme." (§14.3)

### 19.6 Nachtrag: der Verlauf war erfundene Historie

Beim ersten Bau zeichnete der Block „Verlauf aus Statusreports" eine Kurve aus `Wert × [0,72 · 0,81 · 0,91 · 1,0]` — **alle vier Verträge hatten dieselbe Polyline**, weil nur der aktuelle Wert gespeichert war. Beschriftet war der Block als gemessene Historie. Am schärfsten beim Bausparvertrag: „Verlauf aus Statusreports · kein Jahresbericht" mit steigender Kurve darunter, zwei Zeilen unter dem Satz „wird nicht verzinst und nicht geschätzt".

Behoben mit derselben Struktur, die §17.1 für Vermögensstände ohnehin verlangt: **jeder Vertrag führt eine Berichtsreihe** aus Stichtag und Wert, und gezeichnet werden ausschließlich diese Punkte — als Linie mit sichtbaren Messpunkten, darunter erster und letzter Stichtag, im Kopf „N Berichte gespeichert".

Drei Fälle, drei Darstellungen:

| Gespeicherte Berichte | Darstellung |
| --- | --- |
| ≥ 2 | Linie mit Punkten, Stichtage an den Enden |
| 1 | keine Kurve, sondern: „Ein Bericht gespeichert (31.12.2025 · 11.930,40 €) — für einen Verlauf braucht es einen zweiten. Der nächste wird beim Einlesen automatisch angehängt." |
| 0 | Sektion entfällt vollständig |

**Regel: nie eine Kurve aus einem Punkt.** Wo keine Reihe gespeichert ist, gibt es keinen Verlauf — und der Screen sagt das, statt eine Bewegung zu zeichnen, die niemand gemessen hat.

### 19.7 Nachtrag: Bezeichnungen folgen der Vertragsart

Dieselbe Ursache an zweiter Stelle — produktfremde Texte auf allen Arten:

- **Der Wertbestandteil heißt je Art anders**: Rückkaufswert (Kapital-LV), Deckungskapital (Rentenversicherung, Riester, bAV), Sparguthaben (Bausparen). Vorher hieß alles „Rückkaufswert"; ein Bausparvertrag hat keinen.
- **Die Herkunft heißt je Art anders**: „Statusreport 31.07.2025" bei Verträgen mit Jahresbericht, „Auszug 31.12.2025" bei solchen ohne — und die Sektionsüberschrift entsprechend „Verlauf aus Jahresauszügen".
- **Der Ausschluss-Hinweis** zu Bewertungsreserven und Schlussüberschüssen erscheint **nur bei Kapital-LV**. Diese Posten gibt es bei Bausparen und Riester nicht, und ohne Statusreport gibt es auch keinen Bericht, auf den sich der Satz berufen könnte.

Für die Umsetzung: die Bezeichnung ist eine Funktion der Vertragsart, keine Konstante — und das gilt auch für das Formularfeld, das den Wert erfasst.
